  ...
  ${
    user
      ? `<section class="identity"><div class="avatar-large">${accountAvatar(user, 72)}</div><div><h1>${escapeHtml(user.displayName)}</h1><p class="muted">@${escapeHtml(user.handle)} / ${escapeHtml(user.provider)} account</p></div></section>
        ...
        <section class="panel danger"><h2>Danger zone</h2><p class="muted" style="margin-bottom:.75rem">Account deletion is not built yet. FreeIdeaStore now owns its own identity records, so deleting one has to detach or remove the ideas, contributions and reactions attributed to it — this store will not pretend to delete an account while that work stays attached to it.</p><button class="button danger" type="button" disabled>Delete account unavailable here</button></section>`
